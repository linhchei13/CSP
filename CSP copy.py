...
def handle_interrupt(signum, frame):
    ...
    with open(f'results_CSP_{instance_id}.json', 'w') as f:
        json.dump(result, f)
...
def display_solution(bin_width, bin_height, rectangles, bins_assignment, positions, instance_name):
    ...
    plt.tight_layout()
    instance_name = instance_name.split('/')[-1].split('.')[0]
    plt.savefig(f'CSP/{instance_name}.png')
    plt.close()
...
def save_checkpoint(instance_id, variables, clauses, num_bins, status="IN_PROGRESS"):
    ...
    with open(f'checkpoint_CSP_{instance_id}.json', 'w') as f:
...
if __name__ == "__main__":
    ...
    if len(sys.argv) == 1:
        ...
        for instance_id in range(1, len(instances)):
            ...
            for temp_file in [f'results_CSP_{instance_id}.json', f'checkpoint_CSP_{instance_id}.json']:
                if os.path.exists(temp_file):
                    os.remove(temp_file)
            
            # Run instance with runlim
            command = f"./runlim -r {TIMEOUT} python3 CSP.py {instance_id}"
            ...
            try:
                ...
                if os.path.exists(f'results_CSP_{instance_id}.json'):
                    with open(f'results_CSP_{instance_id}.json', 'r') as f:
                        result = json.load(f)
                elif os.path.exists(f'checkpoint_CSP_{instance_id}.json'):
                    with open(f'checkpoint_CSP_{instance_id}.json', 'r') as f:
                        result = json.load(f)
    ...
    # Single instance mode
        ...
        try:
            ...
            # Solve
            optimal_bins, solution = CSP(lower_bound, upper_bound, bin_width, bin_height, rectangles)
