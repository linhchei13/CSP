...
def handle_interrupt(signum, frame):
    ...
    with open(f'results_OR-TOOLS_CP_SB_{instance_id}.json', 'w') as f:
        json.dump(result, f)
...
if not os.path.exists('OR-TOOLS_CP_SB'):
    os.makedirs('OR-TOOLS_CP_SB')
...
def save_checkpoint(instance_id, bins, status="IN_PROGRESS"):
    ...
    with open(f'checkpoint_OR-TOOLS_CP_SB_{instance_id}.json', 'w') as f:
...
def display_solution(W, H, rectangles, positions, assignments, instance_name):
    ...
    plt.savefig(f'OR-TOOLS_CP_SB/{instance_name}.png', dpi=150, bbox_inches='tight')
    plt.close()
...
if __name__ == "__main__":
    ...
    if len(sys.argv) == 1:
        ...
        excel_file = 'OR-TOOLS_CP_SB.xlsx'
        ...
        for instance_id in range(1, len(instances)):
            ...
            for temp_file in [f'results_OR-TOOLS_CP_SB_{instance_id}.json', f'checkpoint_OR-TOOLS_CP_SB_{instance_id}.json']:
                if os.path.exists(temp_file):
                    os.remove(temp_file)
            
            # Run instance with runlim
            command = f"./runlim -r {TIMEOUT} python3 OR-TOOLS_CP_SB.py {instance_id}"
            ...
            try:
                ...
                if os.path.exists(f'results_OR-TOOLS_CP_SB_{instance_id}.json'):
                    with open(f'results_OR-TOOLS_CP_SB_{instance_id}.json', 'r') as f:
                        result = json.load(f)
                elif os.path.exists(f'checkpoint_OR-TOOLS_CP_SB_{instance_id}.json'):
                    with open(f'checkpoint_OR-TOOLS_CP_SB_{instance_id}.json', 'r') as f:
                        result = json.load(f)
