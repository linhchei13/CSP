...
# Create CSP_R_SB folder if it doesn't exist
if not os.path.exists('CSP_R_SB'):
...
def display_solution(bin_width, bin_height, rectangles, bins_assignment, positions, rotations, instance_name):
    ...
    fig.suptitle(f'CSP_R_SB - {instance_name} solution', fontsize=16)
    ...
    for bin_idx, items_in_bin in enumerate(bins_assignment):
        ...
        for item_idx in items_in_bin:
            ...
            rect = plt.Rectangle(positions[item_idx], w, h, 
                               edgecolor="red", facecolor="blue", alpha=0.6)
            ax.add_patch(rect)
        ...
        ax.set_xticks(range(bin_width + 1, bin_width // 100))
        ax.set_yticks(range(bin_height + 1, bin_height // 100))
        ax.set_xlabel('width')
    ...
    plt.tight_layout()
    plt.savefig(f'CSP_R_SB/{instance_name}.png')
    plt.close()
...
def first_fit_upper_bound(rectangles, W, H):
    ...
    for rect in rectangles:
        placed = False
        for bin_idx, bin_rects in enumerate(bins):
            for (rw, rh), rot in [((rect[0], rect[1]), False), ((rect[1], rect[0]), True)]:
                pos = fits(bin_rects, rw, rh, W, H)
                if pos is not None:
                    bin_rects.append((pos[0], pos[1], rw, rh))
                    positions.append((pos[0], pos[1]))
                    rotations.append(rot)
                    item_bin_map.append(bin_idx)
                    placed = True
                    break
            if placed:
                break
        if not placed:
            if rect[0] <= W and rect[1] <= H:
                ...
                item_bin_map.append(len(bins)-1)
                ...
                return float('inf')
    # Prepare bins_assignment for display_solution
    bins_assignment = [[] for _ in range(len(bins))]
    for idx, bin_idx in enumerate(item_bin_map):
        bins_assignment[bin_idx].append(idx)
    # Display solution
    try:
        print("displaying firstfit solution")
        display_solution(W, H, rectangles, bins_assignment, positions, rotations, instance_name=f"{instance_name} - FF" )
    except Exception as e:
        print(f"Error displaying firstfit solution: {e}")
    print(f"Bins: {bins}")
    return len(bins)
...
if __name__ == "__main__":
    ...
    "Hchl7s","Hchl8s", "Hchl9",
        ...
        for instance_id in range(1, len(instances)):
            ...
            command = f"./runlim -r {TIMEOUT} python3 CSP_R_SB.py {instance_id}"
    ...
    # Single instance mode
        ...
        try:
            ...
            # Solve
            optimal_bins = CSP(lower_bound, upper_bound, bin_width, bin_height, rectangles)
...
if __name__ == "__main__":
    ...
    # Single instance mode
        ...
        try:
            ...
            with open(f'results_{instance_id}.json', 'w') as f:
>>>>>>> 95df32ee7e9efa042c45e90c9bda1e6ef4aa078e:CSP_R_SB.py
                json.dump(result, f)