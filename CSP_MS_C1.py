...
def handle_interrupt(signum, frame):
    ...
    with open(f'results_CSP_MS_C1_{instance_id}.json', 'w') as f:
        json.dump(result, f)
...
# Create CSP_MS_C1 folder if it doesn't exist
if not os.path.exists('CSP_MS_C1'):
    os.makedirs('CSP_MS_C1')
...
def display_solution(bin_width, bin_height, rectangles, bins_assignment, positions, instance_name):
    ...
    fig.suptitle(f'CSP_MS_C1 - {instance_name} - {num_bins} bins', fontsize=16)
    ...
    plt.tight_layout()
    plt.savefig(f'CSP_MS_C1/{instance_name}.png')
    plt.close()
...
def save_checkpoint(instance_id, variables, clauses, bins, status="IN_PROGRESS"):
    ...
    with open(f'checkpoint_CSP_MS_C1_{instance_id}.json', 'w') as f:
...
if __name__ == "__main__":
    ...
    if len(sys.argv) == 1:
        ...
        for instance_id in test_instances:
            ...
            if os.path.exists(f'results_CSP_MS_C1_{instance_id}.json'):
                os.remove(f'results_CSP_MS_C1_{instance_id}.json')
            
            # Run the instance with runlim
            command = f"./runlim -r {TIMEOUT} python3 CSP_MS_C1.py {instance_id}"
            try:
                ...
                if os.path.exists(f'results_CSP_MS_C1_{instance_id}.json'):
                    with open(f'results_CSP_MS_C1_{instance_id}.json', 'r') as f:
                        result = json.load(f)
                
                # If no results file, check checkpoint
                elif os.path.exists(f'checkpoint_CSP_MS_C1_{instance_id}.json'):
                    with open(f'checkpoint_CSP_MS_C1_{instance_id}.json', 'r') as f:
                        result = json.load(f)
            ...
            for file in [f'results_CSP_MS_C1_{instance_id}.json', f'checkpoint_CSP_MS_C1_{instance_id}.json']:
    ...
    # Single instance mode
        ...
        try:
            ...
            # Solve using Max-SAT
            optimal_bins, solution = CSP_MaxSAT(rectangles, lower_bound, upper_bound, bin_width, bin_height)
